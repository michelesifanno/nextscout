...
//     const [loadingApi, setLoadingApi] = useState(false); // Stato di caricamento API
...
//             setLoading(true); // Inizia il caricamento generale
...
//                     setLoadingApi(true); // Inizia il caricamento API
...
//                     setLoadingApi(false); // Fine caricamento API
//                 }
//             } catch (err) {
//                 console.error(err);
//                 setLoading(false); // In caso di errore, settiamo `loading` a false
//             }
//         }
...
export default function CareerStats({ stats }) {
    ...
    useEffect(() => {
        async function fetchPlayerOldStats() {
            setLoading(true); // Inizia il caricamento generale
            setLoadingApi(false); // Resetta il caricamento API
            ...
            try {
                ...
                if (
                    ...
                    // 2. Chiamata API per aggiornare i dati
                    const url = `${import.meta.env.VITE_BASE_URL}/players?id=${playerId}&season=${season}`;
                    const response = await axios.get(url, {
                        headers: {
                            "x-rapidapi-host": import.meta.env.VITE_BASE_URL,
                    ...
                    // Aggiorna le stats con i dati ricevuti
                    setOldStats(apiPlayerStats);

                    // 3. Salvataggio nel database per usi futuri
                    const { error: upsertError } = await supabase
                        .from("players_stats_old")
                        .upsert({
                            player_id: apiPlayerData.id,
